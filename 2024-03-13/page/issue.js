...
class IssuePage {
  constructor(driver) {
    ...
    this.issueNameFiel = "//*[@id='field-issues-issueName']";
  ...
  async pressDeleteIssueButton() {
    await this.driver
      .findElement(By.xpath(this.deleteIssueButton))
      .click();
  }

  async enterIssueName(name) {
    await this.driver.findElement(By.xpath(this.issueNameFiel)).sendKeys(name);
  }

  async enterIssueDescription(description) {
    await this.driver
      .findElement(By.xpath(this.issueDescriptionField))
  ...
  async assertCreatedIssueTitleIsVisible(title) {
    const titles = await this.driver.findElement(By.xpath(this.issueTitles));
